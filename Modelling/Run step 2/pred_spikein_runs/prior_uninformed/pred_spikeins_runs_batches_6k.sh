...
tpplc "$MODELS_DIR/predict/optmodel15_Pred_match_uninform.tppl" \
  ...
  --subsample-size 1 \
  --resample manual \
  --output "$BIN_DIR/optmodel15_Pred_match_uninform_2000"

MODEL_BIN="$BIN_DIR/optmodel15_Pred_match_uninform_2000"
ORIG_JSON="$DATA_DIR/modified_pred_in_6k_mcmc.json" #spikein data replaces full dataset, 6k model

SWEEPS=${SWEEPS:-100}
OUT_ROOT="${OUT_ROOT:-$PRED_OUT_DIR/spikeins/uninform/6k}"

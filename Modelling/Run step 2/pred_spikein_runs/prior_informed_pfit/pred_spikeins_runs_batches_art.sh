...
tpplc "$MODELS_DIR/predict/optmodel15_Pred_1_inform_pfit.tppl" \
  -m smc-apf \
  --particles 2000 \
  --subsample \
  --subsample-size 1 \
  --resample manual \
  --output "$BIN_DIR/optmodel15_Pred_1_inform_pfit_2000"

MODEL_BIN="$BIN_DIR/optmodel15_Pred_1_inform_pfit_2000"
ORIG_JSON="$DATA_DIR/modified_pred_in_art_mcmc.json" #spikein data replaces full dataset, art model

SWEEPS=${SWEEPS:-100}
OUT_ROOT="${OUT_ROOT:-$PRED_OUT_DIR/spikeins/inform_pfit/art}"

#!/bin/zsh
# chmod +x extras_pred_fullset_runs_batches_inform_adjust.sh
# ./extras_pred_fullset_runs_batches_inform_adjust.sh

set -euo pipefail

echo "Starting: run model on precomputed per-species batches - inform adjust"

SCRIPT_DIR="$(cd "$(dirname "${(%):-%x}")" && pwd)"
MODELLING_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
source "$MODELLING_ROOT/config.sh"

tpplc "$MODELS_DIR/predict/optmodel15_Pred_match_inform_adjust.tppl" \
  -m smc-apf \
  --particles 100000 \
  --subsample \
  --subsample-size 1 \
  --resample manual \
  --output "$BIN_DIR/optmodel15_Pred_match_inform_adjust_100000"

MODEL_BIN="$BIN_DIR/optmodel15_Pred_match_inform_adjust_100000"
ORIG_JSON="$DATA_DIR/15_hom_priors.json"

SWEEPS=${SWEEPS:-100}
OUT_ROOT="${OUT_ROOT:-$PRED_OUT_DIR/fullset/inform_adjust}"
...
INDEX_LIST=" 7, 8, 18, 31, 84, 115, 121, 142, 151, 189, 190, 192, 203, 255, 257, 272, 273, 275, 283, 295, 347, 348, 349, 350, 352, 366, 371, 384, 392"        # optional explicit list "1, 2, 7 9"

#!/bin/zsh
# chmod +x extras_pred_fullset_runs_batches_uninform.sh
# ./extras_pred_fullset_runs_batches_uninform.sh

set -euo pipefail

echo "Starting: run model on precomputed per-species batches - uninform"

SCRIPT_DIR="$(cd "$(dirname "${(%):-%x}")" && pwd)"
MODELLING_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
source "$MODELLING_ROOT/config.sh"

tpplc "$MODELS_DIR/predict/optmodel15_Pred_match_uninform.tppl" \
  -m smc-apf \
  --particles 100000 \
  --subsample \
  --subsample-size 1 \
  --resample manual \
  --output "$BIN_DIR/optmodel15_Pred_match_uninform_100000"

MODEL_BIN="$BIN_DIR/optmodel15_Pred_match_uninform_100000"
ORIG_JSON="$DATA_DIR/15_hom_priors.json"

SWEEPS=${SWEEPS:-100}
OUT_ROOT="${OUT_ROOT:-$PRED_OUT_DIR/fullset/uninform}"
...
INDEX_LIST="23, 31, 86, 189, 190, 192, 257, 456"        # optional explicit list "1, 2, 7 9"

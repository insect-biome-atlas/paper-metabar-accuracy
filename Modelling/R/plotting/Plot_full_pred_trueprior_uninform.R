...
rds_path        <- file.path(repo_root, "Results/step2_predictions/stitched/fullset/uninform/stiched_pred_fullset.rds")
...
out_dir          <- file.path(repo_root, "Results/plots/fullset/uninform")
...
sample_p <- function(n_spp, seed = 1) {
  set.seed(seed)
  u <- runif(n_spp)
  p <- rbeta(n_spp, 1, 9) #ifelse(u < 0.65, 0.95, rnorm(n_spp, 0.5, 0.1))  # 65% spike at p=0.95; rest ~N(0.5, 0.1)
...
p_all_bar_uninform <- ggplot(overall_long, aes(x = bin, y = prop, fill = source)) +
...
ggsave(file.path(out_dir, paste0("Predict_fullset_summary_", sweep_aggregation, ".jpeg")),
       p_all_bar_uninform, width = 6, height = 5)
...
df_final_uninform               <- df_final
prior_counts_overall_uninform   <- prior_counts_overall
